...
class StockMoveConteo(models.Model):
    ...
    def _compute_conteo(self):
    	...
    	if(def_pro==search_pro):
    		sql = "select sm.product_uom_qty, sp.name, sl.name from stock_move sm full outer join stock_picking sp on sp.id=sm.picking_id full outer join stock_location sl on sl.id = sm.location_id WHERE sm.product_id='"+str(def_pro)+"' and sm.id<='"+str(self.id)+"' and sm.state='done';"
	        cr.execute(str(sql))
	        filas = cr.fetchall()
	        if(filas!=[]):
	            for c in filas:
